...
This diagram represents the key functions, inputs, and outputs related to normal hardware function of the VxScan precinct scanner.  

It does *not* include:
* sequential connections that depend on user actions (e.g. hiding the administrative access features coming before collapsing the system)
* digital connections among components (e.g. the CPU sends and receives signals from all electrical components)
* other inputs/outputs that are less important or not required for using the system (e.g. dust, debris, gravity, ESD)
...
%%            i24.04b.1("human force") --> f24.04.1 
    ...
    subgraph stability["Connection to Ballot Receptacle"]
        ...
        f16.07["Transfer forces and accepted
...
%%    f22.04 -..-> f9.02
    ...
    f22.03 ---> administrativeAccess
    f22.03 ---> paperPath
    f22.03 ---> digitalInteractions

    f14.06 ---> administrativeAccess
    f14.06 ---> paperPath
    f14.06 ---> digitalInteractions
